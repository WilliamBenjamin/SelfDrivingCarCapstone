...
def get_box_class(boxes, labels, obj_thresh, quiet=True):
    classifications = []
    scores = []
    for box in boxes:
        ...
        for i in range(len(labels)):
            if box.classes[i] > obj_thresh:
                classifications.append(str(labels[i]))
                scores.append(box.classes[i])
                if not quiet:
                    print("The detected class is {} with score {}".format(labels[i],box.classes[i] ))
